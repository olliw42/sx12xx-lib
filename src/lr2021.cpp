...
void Lr2021DriverBase::SpiRead(uint8_t *datain, uint8_t len) {
  uint8_t dummy = 0; // NOP
...
void Lr2021DriverBase::SpiWrite(uint8_t *dataout, uint8_t len) {
  uint8_t dummy;
...
void Lr2021DriverBase::WriteCommand(uint16_t opcode, uint8_t *data,
                                    uint8_t len) {
  WaitOnBusy();
  SpiSelect();
...
void Lr2021DriverBase::ReadCommand(uint16_t opcode, uint8_t *data,
                                   uint8_t len) {
  WaitOnBusy();
  SpiSelect();
  SpiTransfer((uint8_t)((opcode & 0xFF00) >> 8), &_status1);
  SpiTransfer((uint8_t)(opcode & 0x00FF), &_status2);
  if (opcode != LR2021_CMD_GET_STATUS) { // not needed for get status
    SpiDeselect();
    WaitOnBusy();
    SpiSelect();
  }
...
void Lr2021DriverBase::GetStatus(uint8_t *Status1, uint8_t *Status2) {
  WriteCommand(LR2021_CMD_GET_STATUS); // don't need a response, so don't need
                                       // to use ReadCommand
...
void Lr2021DriverBase::GetLastStatus(uint8_t *Status1, uint8_t *Status2) {
  *Status1 = _status1;
  *Status2 = _status2;
}

void Lr2021DriverBase::SetStandby(uint8_t StandbyConfig) {
  WriteCommand(LR2021_CMD_SET_STANDBY, StandbyConfig);
}

void Lr2021DriverBase::SetPacketType(uint8_t PacketType) {
  WriteCommand(LR2021_CMD_SET_PACKET_TYPE, PacketType);
}

void Lr2021DriverBase::SetRfFrequency(uint32_t RfFrequency) {
  uint8_t buf[4];
  ...
  WriteCommand(LR2021_CMD_SET_RF_FREQUENCY, buf, 4);
}

void Lr2021DriverBase::SetDioRfSwitchConfig(uint8_t Dio, uint8_t Config) {
  uint8_t buf[2];
  ...
  WriteCommand(LR2021_CMD_SET_DIO_AS_RF_SWITCH, buf, 2);
}

void Lr2021DriverBase::SetTcxoMode(uint8_t OutputVoltage, uint32_t Delay) {
  uint8_t buf[5];
  ...
  WriteCommand(LR2021_CMD_SET_TCXO_MODE, buf, 5);
}

void Lr2021DriverBase::SetModulationParams(uint8_t SpreadingFactor,
                                           uint8_t Bandwidth,
                                           uint8_t CodingRate,
  ...
  WriteCommand(LR2021_CMD_SET_MODULATION_PARAMS, buf, 2);
}

void Lr2021DriverBase::SetPacketParams(uint16_t PreambleLength,
                                       uint8_t HeaderType,
  ...
  WriteCommand(LR2021_CMD_SET_PACKET_PARAMS, buf, 4);
}

void Lr2021DriverBase::SetDioIrqConfig(uint8_t Dio, uint32_t IrqMask) {
  uint8_t buf[5];
  ...
  WriteCommand(LR2021_CMD_SET_DIO_IRQ_CONFIG, buf, 5);
}

uint32_t Lr2021DriverBase::GetIrqStatus() {
  uint8_t status[4];

  ReadCommand(LR2021_CMD_GET_STATUS, status, 4);

  return (uint32_t)((status[0] << 24) | (status[1] << 16) | (status[2] << 8) |
                    (status[3]));
}

void Lr2021DriverBase::ClearIrq(uint32_t IrqToClear) {
  uint8_t buf[4];
  ...
  WriteCommand(LR2021_CMD_CLEAR_IRQ, buf, 4);
}

uint32_t Lr2021DriverBase::GetAndClearIrqStatus(uint32_t IrqToClear) {
...
void Lr2021DriverBase::SetPaConfig(uint8_t pa_sel, uint8_t pa_lf_mode,
  ...
  WriteCommand(LR2021_CMD_SET_PA_CONFIG, buf, 3);
}

void Lr2021DriverBase::SetTxParams(uint8_t Power, uint8_t RampTime) {
  uint8_t buf[2];

  buf[0] = Power;
  buf[1] = RampTime;

  WriteCommand(LR2021_CMD_SET_TX_PARAMS, buf, 2);
}

void Lr2021DriverBase::SetTx(uint32_t TxTimeout) {
  uint8_t buf[3];
  ...
  WriteCommand(LR2021_CMD_SET_TX, buf, 3);
}

// Rx methods

void Lr2021DriverBase::SetRx(uint32_t RxTimeout) {
  uint8_t buf[3];
  ...
  WriteCommand(LR2021_CMD_SET_RX, buf, 3);
}

void Lr2021DriverBase::GetPacketStatus(int16_t *RssiSync, int8_t *Snr) {
  uint8_t status[8];
  ...
  ReadCommand(LR2021_CMD_GET_PACKET_STATUS, status, 8);
...
void Lr2021DriverBase::SetRegMode(uint8_t RegModeParam) {
  uint8_t buf[1];

  buf[0] = RegModeParam;

  WriteCommand(LR2021_CMD_SET_REG_MODE, buf, 1);
}

void Lr2021DriverBase::SetRxTxFallbackMode(uint8_t FallbackMode) {
  uint8_t buf[1];

  buf[0] = FallbackMode;

  WriteCommand(LR2021_CMD_SET_RXTX_FALLBACK_MODE, buf, 1);
}

void Lr2021DriverBase::SetFs(void) { WriteCommand(LR2021_CMD_SET_FS); }

void Lr2021DriverBase::SetRxPath(uint8_t rx_path, uint8_t rx_boost) {
  uint8_t buf[2];

  buf[0] = rx_path;
  buf[1] = rx_boost;

  WriteCommand(LR2021_CMD_SET_RX_PATH, buf, 2);
}

void Lr2021DriverBase::SetLoraCadParams(uint8_t NbSymbols, uint8_t DetPeak,
  ...
  WriteCommand(LR2021_CMD_SET_LORA_CAD_PARAMS, buf, 7);
}

void Lr2021DriverBase::SetLoraCad(void) {
  WriteCommand(LR2021_CMD_SET_LORA_CAD);
}

void Lr2021DriverBase::CalibFE(uint32_t Freq, uint8_t Path) {
  uint8_t buf[6];
  ...
  WriteCommand(LR2021_CMD_CALIB_FE, buf, 6);
}

void Lr2021DriverBase::ClearErrors(void) {
  WriteCommand(LR2021_CMD_CLEAR_ERRORS);
}

void Lr2021DriverBase::EnableSx127xCompatibility(void) {
  ...
  WriteCommand(LR2021_CMD_WRITE_REG_MEM_MASK_32, buf, 11);
}

// GFSK methods

void Lr2021DriverBase::SetModulationParamsGFSK(uint32_t br_bps,
                                               uint8_t PulseShape,
                                               uint8_t Bandwidth,
                                               uint32_t Fdev_hz) {
  uint8_t buf[9];
  ...
  WriteCommand(LR2021_CMD_SET_MODULATION_PARAMS_GFSK, buf, 9);
}

void Lr2021DriverBase::SetPacketParamsGFSK(uint16_t PreambleLength,
  ...
  WriteCommand(LR2021_CMD_SET_PACKET_PARAMS_GFSK, buf, 7);
}

void Lr2021DriverBase::SetSyncWordGFSK(uint16_t SyncWord) {
  uint8_t cmd_buf[10];
  ...
  WriteCommand(LR2021_CMD_SET_GFSK_SYNC_WORD, cmd_buf, 10);
}

void Lr2021DriverBase::GetPacketStatusGFSK(int16_t *RssiSync) {
  uint8_t status[5];

  // position 0 is status1, again

  ReadCommand(LR2021_CMD_GET_PACKET_STATUS_GFSK, status, 3);
...
void Lr2021DriverBase::GetVersion(uint8_t *HwVersion, uint8_t *UseCase,
                                  uint8_t *FwMajor, uint8_t *FwMinor) {
  uint8_t version[5];

  // position 0 is status1, again

  ReadCommand(LR2021_CMD_GET_VERSION, version, 5);
...
void Lr2021DriverBase::SetModulationParamsFLRC(uint8_t BitrateBw,
                                               uint8_t CodingRate,
                                               uint8_t PulseShape) {
  uint8_t buf[2];
  ...
  WriteCommand(LR2021_CMD_SET_MODULATION_PARAMS_FLRC, buf, 2);
}

void Lr2021DriverBase::SetPacketParamsFLRC(uint8_t AgcPblLen,
                                           uint8_t SyncWordLength,
                                           uint8_t SyncWordTx,
  ...
  WriteCommand(LR2021_CMD_SET_PACKET_PARAMS_FLRC, buf, 4);
}

void Lr2021DriverBase::SetSyncWordFLRC(uint8_t SyncWordNum, uint32_t SyncWord) {
  uint8_t buf[5];
  ...
  WriteCommand(LR2021_CMD_SET_FLRC_SYNCWORD, buf, 5);
}

void Lr2021DriverBase::GetPacketStatusFLRC(int16_t *RssiSync) {
  uint8_t status[3];

  ReadCommand(LR2021_CMD_GET_PACKET_STATUS_FLRC, status, 3);
...
void Lr2021DriverBase::WriteRadioTxFifo(uint8_t *data, uint8_t len) {
  // Direct FIFO write using opcode 0x0002
  WaitOnBusy();
  SpiSelect();
  SpiTransfer((uint8_t)((LR2021_CMD_WRITE_RADIO_TX_FIFO & 0xFF00) >> 8),
              &_status1);
  SpiTransfer((uint8_t)(LR2021_CMD_WRITE_RADIO_TX_FIFO & 0x00FF), &_status2);
  SpiWrite(data, len);
  SpiDeselect();
}

void Lr2021DriverBase::ReadRadioRxFifo(uint8_t *data, uint8_t len) {
  // Direct FIFO read using opcode 0x0001
  WaitOnBusy();
  SpiSelect();
  SpiTransfer((uint8_t)((LR2021_CMD_READ_RADIO_RX_FIFO & 0xFF00) >> 8),
              &_status1);
  SpiTransfer((uint8_t)(LR2021_CMD_READ_RADIO_RX_FIFO & 0x00FF), &_status2);
  SpiRead(data, len);
  SpiDeselect();
}

uint16_t Lr2021DriverBase::GetRxFifoLevel(void) {
  uint8_t level[3];

  ReadCommand(LR2021_CMD_GET_RX_FIFO_LEVEL, level, 3);

  // position 0 is status1
  return ((uint16_t)level[1] << 8) | level[2];
}

uint16_t Lr2021DriverBase::GetTxFifoLevel(void) {
  uint8_t level[3];

  ReadCommand(LR2021_CMD_GET_TX_FIFO_LEVEL, level, 3);

  // position 0 is status1
  return ((uint16_t)level[1] << 8) | level[2];
}

void Lr2021DriverBase::ClearRxFifo(void) {
  WriteCommand(LR2021_CMD_CLEAR_RX_FIFO);
}

void Lr2021DriverBase::ClearTxFifo(void) {
  WriteCommand(LR2021_CMD_CLEAR_TX_FIFO);
}